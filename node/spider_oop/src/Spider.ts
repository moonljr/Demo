 ...
 * 然后 在子类中处理的到的结果即可
 * 爬虫用法 创建爬虫对象 传入URL 自动开爬
 */
const http = require('http')
import SpiderOptions from './interfaces/SpiderOptions'

// 将 Spider 改成抽象类 在子类中实现抽象方法 onCatchHTML 来处理爬取的HTML
abstract class Spider {
    ...
    let req = http.request(
      ...
      (res: any) => {
        ...
        res.on('end', () => {
          ...
          /**
           * 抽象方法调用 子类需要做的事  父类(超类) 不需要知道子类是怎么实现的  只需要调用一下抽象方法
           * 具体的实现由子子孙孙继承时实现即可
           */
  ...
  abstract onCatchHTML(resut: string): any
}
